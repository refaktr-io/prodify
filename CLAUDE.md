...
- `usage.py` counts events (`uploads` in `upload_request.py`, `conversions`/`rejected` in `generator.py`) with atomic `ADD`s on two DynamoDB items, `total` and `day#YYYY-MM-DD`; it's best-effort and never raises. `stats.py` serves them at `GET /stats` (public, `Cache-Control: max-age=60`) for the website's counter. Counting is a no-op when `USAGE_TABLE` is unset (tests).
...
- Create: download the presigned `SourceZipUrl` (~2 h), use `dist/`, `build/`, or `.output/public/` if one contains an `index.html`, else `bun install && bun run build` (npm if no bun lockfile; never `--omit=dev` — build tools are devDependencies; caches redirected to `/tmp` because the filesystem is read-only), then sync to the bucket with `Cache-Control` (`no-cache` HTML, `immutable` `assets/*`) and delete stale keys.
- **TanStack Start (Lovable's current template) builds a Cloudflare Worker by default.** Before building, `ensure_prerender()` patches the *build copy's* `vite.config.ts` to add TanStack Start's `prerender` option (`inject_prerender` recognizes Lovable's `defineConfig({ tanstackStart: {...} })` wrapper and plain `tanstackStart(...)` calls), so the build also writes a static site to `.output/public/`. `NITRO_PRESET=static` does not work with this toolchain; don't try that route again.
